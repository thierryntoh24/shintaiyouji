...
export default function TrueSolarTimePage() {
  ...
  const { status, active, activeRes: res, setActiveRes, isExpanded } =
    useAppUI();
  ...
  const tickerRef = useRef<ReturnType<typeof requestAnimationFrame> | null>(null);
  ...
  if (status !== "ready" || !active)
    return <LoadingUI message={{ description: "Getting everything ready..." }} />;
  ...
  const solarTimes: SolarTimes = getSolarTimes(now, active.latitude, active.longitude);

  /** Current sun position (altitude, azimuth, distance, etc.). */
  const sunPos: SunPosition = getSunPosition(now, active.latitude, active.longitude);

  /** Current moon position (altitude, azimuth, distance). */
  const moonPos: MoonPosition = getMoonPosition(now, active.latitude, active.longitude);
  ...
  const moonTimes: MoonTimes = getMoonTimes(now, active.latitude, active.longitude);
  ...
  const eveningGoldenHourStart = makeSolarEntry(solarTimes.eveningGoldenHourStart);
  ...
  return (
    <div className="w-full h-screen overflow-hidden frosted-bg frosted-bg-xl [--frosted-image:url('https://i.pinimg.com/1200x/e2/22/d1/e222d106f4534f2f5c525de73baa4ce4.jpg')]!">
      ...
      <main className="grid grid-rows-[auto_auto_1fr] w-full gap-5 h-[calc(100svh-(var(--header-height)*2))]!">
        ...
        <div className="neue-grid justify-items-stretch tabular-nums tracking-tight w-full">
          <div
            className={cn(
              "col-span-6 col-start-2 flex items-center gap-4 justify-between font-serif font-normal",
              "transition-[font-size] duration-500 ease-in-out",
              isExpanded ? "text-5xl" : "text-9xl",
            )}
          >
            <span className="flex items-start gap-0.5">
              {hourFormat === "12" && parts?.period && (
                <span
                  className={cn(
                    "font-sans opacity-0 font-medium not-italic text-nowrap",
            ...
            <span className="flex items-start gap-0.5">
              ...
              {hourFormat === "12" && parts?.period && (
                <span
                  className={cn(
                    "font-sans font-medium not-italic text-nowrap",
                    isExpanded ? "text-xs" : "text-sm",
                  )}
                >
                  [ {parts.period} ]
        ...
        <div className="neue-grid w-full self-start overflow-y-auto">
          ...
          <div className="col-span-3 col-start-2 h-fit flex flex-col gap-1">
            ...
            >
              ...
              {moonrise && (
                <DataRow label="Moonrise" value={fmtEntry(moonrise, hourFormat)} />
              )}
              {moonset && (
                <DataRow label="Moonset" value={fmtEntry(moonset, hourFormat)} />
              )}
            </div>
          </div>
          ...
          <div className="col-span-3 flex flex-col gap-1">
            ...
            >
              ...
              {civilDawn && (
                <DataRow label="Civil dawn" value={fmtEntry(civilDawn, hourFormat)} />
              )}
              {civilDusk && (
                <DataRow label="Civil dusk" value={fmtEntry(civilDusk, hourFormat)} />
              )}
              {nauticalDawn && (
                <DataRow
                  label="Nautical dawn"
 ...
 */
function fmtEntry(entry: SolarEventEntry, hourFormat: HourFormat): string {
  const fmt = (t: FormattedTime) =>
    `${t.hh} : ${t.mm}${hourFormat === "12" && t.period ? ` ${t.period}` : ""}`;
  return `${fmt(entry.solar)}  (${entry.offset} → ${fmt(entry.local)})`;
}
...
function DataRow({ label, value }: { label: string; value: string }) {
  return (
    <div className="grid grid-cols-3 gap-5">
      ...
      <div className="col-span-2 tabular-nums">{value}</div>
    </div>
  );
}
